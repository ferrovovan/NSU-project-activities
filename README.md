...
### Задача 3 — **CV-3-20**
**Смысл:**  
Интеграция предыдущих решений по обработке изображений: распознавание объектов на складе по цвету и размеру.  
Задача учит строить конвейер обработки изображений: поиск контуров, классификация по размеру и цвету, формирование отчёта.  

**Описание:**  
На входе изображение склада с объектами.  
- Выделяются контуры объектов с фильтрацией по площади (малый/средний/большой).  
- Определяется преобладающий цвет объектов (красный, синий, зелёный, жёлтый, оранжевый, белый, серый, чёрный).  
- Форма пока не распознаётся (placeholder).  
- Составляется отчёт в CSV с информацией о каждом объекте: идентификатор, цвет, размер, форма, площадь.  
- Реализован синтетический пример для проверки конвейера обработки и визуализации выделенных объектов.
...
`merge_branches.sh` - **не работает** по-умному.  
С ветками полный кавардак - прилично в `main` не сольёшь.  
Всё не для людей сделали.  
